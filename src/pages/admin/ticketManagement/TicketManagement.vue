...
const loadTicketsByMatch = async (maTranDau) => {
  try {
    const res = await axios.get(
      `${import.meta.env.VITE_API_BE_BASE_URL}/ve/trandau/${maTranDau}`
    );
...
const calculateStats = () => {
  ...
  const revenue = tickets.value.reduce(
    (sum, v) => (v.trangThai === "da_ban" ? sum + v.giaVe : sum),
    0
  );
  stats.value = {
  ...
  <div class="container-fluid py-4">
    ...
    <MatchSelector
      ...
      @select="
        (match) => {
          selectedMatch = match;
          loadTicketsByMatch(match.maTranDau);
        }
      "
    />
    ...
    <TicketForm
      v-if="selectedMatch"
      :maTranDau="selectedMatch.maTranDau"
    ...
    <TicketList
      v-if="selectedMatch"
      :tickets="tickets"
      @updated="loadTicketsByMatch(selectedMatch.maTranDau)"
